                ...
                {countries.map((c) => {
                  ...
                  return (
                    <tr key={c.name} className={"border-b border-gray-100 " + (selected.name === c.name ? "bg-orange-50" : "")}>
                      ...
                      <td className="py-3 text-right font-semibold" style={{color:"#111827"}}>${Math.round(fireUSD).toLocaleString()}</td>
                    </tr>
                  )
                })}
              </tbody>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-600 text-sm">Your FIRE number depends on your annual spending. Living in Thailand at $1,200/month requires a much smaller portfolio than living in France at $2,500/month. The safe withdrawal rate also varies — we use 3.5% for higher-cost countries and 4% for lower-cost ones.</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-gray-600 text-sm">EU countries offer public healthcare access for residents. In Thailand, private insurance runs $100-200/month. Always factor in healthcare costs when calculating your budget abroad.</p>
            </div>
          </div>
        </div>
      </section>
...
"@
[System.IO.File]::WriteAllText("C:\Users\Utilisateur\Desktop\firepathtools\app\fire-by-country-calculator\client.tsx", $part1, [System.Text.Encoding]::UTF8)