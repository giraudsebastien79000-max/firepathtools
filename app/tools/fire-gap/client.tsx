    ...
    <main className="min-h-screen bg-[#030712] text-white">
      <div className="max-w-3xl mx-auto px-4 py-16">
        ...
        <div className="mb-10">
          ...
          <p className="text-gray-400 text-lg">Find out exactly how far you are from financial independence — and which lever closes the gap fastest.</p>
        </div>
          ...
          <div className="mt-8 space-y-6">
            ...
            <div className="bg-gray-900 rounded-2xl p-8">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-gray-800 rounded-xl p-4">
                  <p className="text-gray-400 text-sm">Current Gap</p>
            ...
            <div className="bg-gray-900 rounded-2xl p-8">
              ...
              <p className="text-gray-400 text-sm mb-6">See how small changes today dramatically change your FIRE timeline.</p>
              <div className="space-y-4">

                <div className="bg-gray-800 rounded-xl p-5">
                  <div className="flex items-center justify-between mb-2">
                    <p className="font-semibold text-white">Reduce expenses by $300/month</p>
                    <span className={`text-sm font-bold px-3 py-1 rounded-full ${result.scenarioExpenses > 0 ? "bg-green-900/50 text-green-400" : "bg-gray-700 text-gray-400"}`}>
                      {result.scenarioExpenses > 0 ? `-${result.scenarioExpenses} years` : "No change"}
                    </span>
                  </div>
                  <p className="text-gray-400 text-sm">New FIRE timeline: {result.yearsExpenses === 999 ? "100+" : result.yearsExpenses} years → FIRE at {result.yearsExpenses === 999 ? "—" : currentAge + result.yearsExpenses}</p>
                </div>

                <div className="bg-gray-800 rounded-xl p-5">
                  <div className="flex items-center justify-between mb-2">
                    <p className="font-semibold text-white">Save $300/month more</p>
                    <span className={`text-sm font-bold px-3 py-1 rounded-full ${result.scenarioSavings > 0 ? "bg-green-900/50 text-green-400" : "bg-gray-700 text-gray-400"}`}>
                      {result.scenarioSavings > 0 ? `-${result.scenarioSavings} years` : "No change"}
                    </span>
                  </div>
                  <p className="text-gray-400 text-sm">New FIRE timeline: {result.yearsSavings === 999 ? "100+" : result.yearsSavings} years → FIRE at {result.yearsSavings === 999 ? "—" : currentAge + result.yearsSavings}</p>
                </div>

                <div className="bg-gray-800 rounded-xl p-5">
                  <div className="flex items-center justify-between mb-2">
                    <p className="font-semibold text-white">Increase return by 1%</p>
                    <span className={`text-sm font-bold px-3 py-1 rounded-full ${result.scenarioReturn > 0 ? "bg-green-900/50 text-green-400" : "bg-gray-700 text-gray-400"}`}>
                      {result.scenarioReturn > 0 ? `-${result.scenarioReturn} years` : "No change"}
                    </span>
                  </div>
                  <p className="text-gray-400 text-sm">New FIRE timeline: {result.yearsReturn === 999 ? "100+" : result.yearsReturn} years → FIRE at {result.yearsReturn === 999 ? "—" : currentAge + result.yearsReturn}</p>
                </div>
