...
export default function FIREProgressClient() {
  ...
  const MilestoneRow = ({ label, amount, reached }) => (
    <div className={`flex justify-between items-center p-3 rounded-xl border ${reached ? "bg-green-900/20 border-green-400/30" : "bg-slate-800 border-slate-600"}`}>
      <span className={`text-sm ${reached ? "text-green-400" : "text-gray-300"}`}>{label}</span>
    ...
    <main className="min-h-screen bg-gray-950 text-white flex flex-col items-center justify-center px-4 py-16">
      <div className="max-w-xl w-full">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-300 text-base">Track exactly where you are on your path to financial independence.</p>
        </div>
          ...
          {result !== null && (
            <div className="space-y-4">
              <div className={`rounded-xl p-6 text-center border ${result.done ? "bg-green-900/30 border-green-400/30" : "bg-slate-800 border-orange-400/30"}`}>
                <p className="text-gray-300 text-sm mb-1">FIRE Progress</p>
                <p className="text-6xl font-bold text-orange-400">{result.pct}%</p>
                {!result.done && <p className="text-gray-300 text-sm mt-2">FIRE in {result.years} years ({result.months} months)</p>}
              ...
              <div className="space-y-2">
                <p className="text-gray-300 text-xs uppercase tracking-widest">Milestones</p>
