    ...
    <main className="min-h-screen bg-gray-950 text-white flex flex-col items-center justify-center px-4 py-16">
      <div className="max-w-xl w-full">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-300 text-base">See exactly when you will be debt-free and how much interest you can save.</p>
        </div>
        <div className="bg-gray-900 rounded-2xl p-8 space-y-6 border border-gray-800">
          <div>
            ...
            <input type="number" value={balance} onChange={(e) => setBalance(e.target.value)} placeholder="e.g. 15000" className="w-full bg-slate-800 border border-slate-600 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-orange-400" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Annual Interest Rate (%)</label>
            <input type="number" value={rate} onChange={(e) => setRate(e.target.value)} placeholder="e.g. 19.99" className="w-full bg-slate-800 border border-slate-600 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-orange-400" />
          </div>
          <div>
          ...
          {result !== null && !result.error && (
            <div className="space-y-4">
              <div className="bg-slate-800 rounded-xl p-6 text-center border border-orange-400/30">
                ...
                <p className="text-5xl font-bold text-orange-400">{result.months} <span className="text-2xl text-gray-300">months</span></p>
                <p className="text-gray-300 text-sm mt-2">That is {result.years} years</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-800 rounded-xl p-4 border border-slate-600 text-center">
                  <p className="text-gray-300 text-xs mb-1">Total Interest Paid</p>
                  <p className="text-xl font-bold text-red-400">${result.totalInterest.toLocaleString("en-US", { maximumFractionDigits: 0 })}</p>
                </div>
                <div className="bg-slate-800 rounded-xl p-4 border border-slate-600 text-center">
                  <p className="text-gray-300 text-xs mb-1">Total Amount Paid</p>
