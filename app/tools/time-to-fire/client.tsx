        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div>
            ...
            <input type="number" value={age} onChange={e => setAge(Number(e.target.value))}
              className="w-full bg-slate-800 border border-slate-600 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-orange-500" />
          </div>
          <div>
            <label className="block text-sm text-gray-300 mb-2">Annual Income (after tax)</label>
          ...
          <div>
            <label className="block text-sm text-gray-300 mb-2">Current Savings / Portfolio</label>
            <input type="number" value={savings} onChange={e => setSavings(Number(e.target.value))}
              className="w-full bg-slate-800 border border-slate-600 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-orange-500" />
          </div>
          <div className="sm:col-span-2">
            <label className="block text-sm text-gray-300 mb-2">Expected Annual Return: {returnRate}%</label>
          ...
          <div className="mt-6 space-y-4">
            {result.years === -1 ? (
              <div className="bg-red-900/30 border border-red-500/30 rounded-xl p-6 text-center">
                ...
                <p className="text-gray-300 text-sm mt-2">Increase income or reduce expenses to reach FIRE.</p>
              </div>
            ) : (
              <>
                <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-6 text-center">
                  ...
                  <p className="text-gray-300 mt-2">You will reach FIRE at age <span className="text-white font-bold">{result.fireAge}</span></p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="bg-slate-800 rounded-xl p-5 text-center">
                    <p className="text-gray-300 text-sm mb-1">FIRE Number</p>
